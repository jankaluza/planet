...
public:
	...
	INDEX LockElement()
	{
		...
		if(this->m_nLockedElements < this->m_nElements)
		{
			...
			this->m_pFlags[nElement] = CArray<NODE, INDEX>::Locked;
		}
		return nElement;
	}
...
public:
	...
	INDEX LockElement()
	{
		...
		if(m_nLowestUnused < this->m_nElements)
		{
			...
			while(m_nLowestUnused < this->m_nElements && (this->m_pFlags[m_nLowestUnused] & CArray<NODE, INDEX>::Locked))
				m_nLowestUnused++;
		}
		return nElement;
	}
	void UnlockElement(INDEX n)
	{
		this->m_pFlags[n] &= ~CArray<NODE, INDEX>::Locked;
		this->m_nLockedElements--;
...
public:
	...
	// Members to find items in the list by ID, name, or index
	A *FindID(unsigned long dwID, A *pNode=NULL)
	{
		if(!pNode)
			pNode = GetHead();
...
public:
	...
	bool IsInTree()						{ return (this->m_pParent != NULL); }
	...
	bool Remove()
	{
		if(this->m_pChild[Left] && this->m_pChild[Right])
			MoveDown((A *)this);

		int nDirection = (this->m_pChild[Left] != NULL) ? Left : Right;
		A *pParent = this->m_pParent;
		A *pNode = this->m_pChild[nDirection];
		this->m_pParent = NULL;
		this->m_pChild[nDirection] = NULL;
