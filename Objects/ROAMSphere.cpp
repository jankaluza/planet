...
// Merges objects from FROM triangle into TO triangle only if object belongs
// to the TO triangle. If the merge for particular object happens, the object
// is removed from FROM triangle. Otherwise the object is kept unchanged in the
// FROM triangle.
static inline void MERGE(CROAMTriangle *FROM, CROAMTriangle *TO) {
	std::vector<Planet2::Object *> _to_erase;
	std::vector<Planet2::Object *> objs = FROM->objects;
	for (std::vector<Planet2::Object *>::const_iterator obj = objs.begin(); obj != objs.end(); ++obj) { 
		if (std::find(TO->objects.begin(), TO->objects.end(), *obj) == TO->objects.end()) {
			CVector _p;
			CVector _f((*obj)->GetPosition().x, (*obj)->GetPosition().y, (*obj)->GetPosition().z);
			_f = _f * -1;
			if (TO->CheckIntersect(_f, _p)) {
				TO->objects.push_back(*obj);
				_to_erase.push_back(*obj);
			}
		}
	}
	while (!_to_erase.empty()) {
		FROM->objects.erase(std::find(FROM->objects.begin(), FROM->objects.end(), _to_erase.front()));
		_to_erase.erase(_to_erase.begin());
	}
}
...
bool CROAMSphere::Split(CROAMTriangle *pTriangle)
{
	...
	//pNewTriangle[0]->objects = pTriangle->objects;
...
#define SPLIT_NORMAL	// Updates the normal of all vertices affected by the split
	...
	MERGE(pTriangle, pNewTriangle[0]);
	MERGE(pOpposite, pNewTriangle[1]);

	onTriangleUpdated(pNewTriangle[0]);
	onTriangleUpdated(pNewTriangle[1]);
	onTriangleUpdated(pTriangle);
	onTriangleUpdated(pOpposite);
...
void CROAMSphere::Merge(CROAMDiamond *pDiamond)
{
	...
	for(int i=0; i<2; i++)
	{
		...
		onTriangleUpdated(p[0]);
		//onTriangleUpdated(p[1]);
		//onTriangleUpdated(p[2]);
		//onTriangleUpdated(p[3]);
		delete pChild;
...
CROAMTriangle *CROAMSphere::getCords(CVector v, CVector &point) {
	// This is naive and slow, but it's called only when new object is added
	// during Planet initialization, so it's OK.
	for(int nFace=0; nFace<6; nFace++)
	{
		for(CROAMTriangle *pTriangle=m_llTriangle[nFace].GetHead(); pTriangle->IsInList(); pTriangle = pTriangle->GetNext())
		{
			if (pTriangle->CheckIntersect(v, point)) {
				return pTriangle;
			}
		}
	}
